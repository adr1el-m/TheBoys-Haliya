...
import { 
  Calendar, Clock, User, Check, X, RefreshCw, AlertTriangle,
  Building2, LogOut, Bell, Search, Eye, Activity, Shield
} from 'lucide-react';
...
export default function FacilityDashboard() {
  ...
  const avgScore = appointments.length > 0 ? (appointments.reduce((sum: number, a: any) => sum + (a.triage_score || 0), 0) / appointments.length).toFixed(1) : '—';
    ...
    <main className="min-h-screen bg-slate-50 flex">
      ...
      <aside className="w-72 bg-white border-r border-slate-100 p-8 hidden lg:flex flex-col gap-8">
        <div className="flex items-center gap-2">
          <div className="bg-blue-600 p-2 rounded-xl text-white shadow-lg shadow-blue-100"><Building2 size={20} /></div>
          <span className="text-xl font-black tracking-tighter text-slate-800">HALIYA HUB</span>
        </div>

        {/* Provider Profile Card */}
        <div className="bg-slate-50 rounded-2xl p-5 space-y-3 border border-slate-100">
          <div className="w-12 h-12 bg-blue-100 text-blue-700 rounded-xl flex items-center justify-center font-black text-lg">
            {user?.name?.[0]?.toUpperCase() || 'F'}
          </div>
          <div>
            <p className="font-bold text-slate-800 text-sm">{user?.name}</p>
            <p className="text-[11px] text-slate-400 font-medium">Healthcare Provider</p>
          </div>
          <div className="grid grid-cols-3 gap-1 pt-2 border-t border-slate-200">
            <div className="text-center"><p className="text-lg font-black text-blue-600">{appointments.length}</p><p className="text-[9px] font-bold text-slate-400 uppercase">Queue</p></div>
            <div className="text-center"><p className="text-lg font-black text-amber-500">{pending}</p><p className="text-[9px] font-bold text-slate-400 uppercase">Pending</p></div>
            <div className="text-center"><p className="text-lg font-black text-emerald-500">{confirmed}</p><p className="text-[9px] font-bold text-slate-400 uppercase">Done</p></div>
          </div>
        </div>
        
        <nav className="flex-1 space-y-2">
          <button className="w-full flex items-center gap-3 p-4 bg-blue-50 text-blue-700 rounded-2xl font-bold transition-all"><Calendar size={20} />Queue Manager</button>
          <Link href="/dashboard" className="flex items-center gap-3 p-4 text-slate-500 hover:bg-slate-50 rounded-2xl font-bold transition-all"><AlertTriangle size={20} />Health Alerts</Link>
        </nav>
      ...
      <div className="flex-1 p-6 md:p-12 overflow-y-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm"><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Total Queue</p><p className="text-3xl font-black text-slate-900 mt-1">{appointments.length}</p></div>
          <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm"><p className="text-[10px] font-black uppercase tracking-widest text-amber-500">Pending Review</p><p className="text-3xl font-black text-amber-600 mt-1">{pending}</p></div>
          <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm"><p className="text-[10px] font-black uppercase tracking-widest text-emerald-500">Confirmed</p><p className="text-3xl font-black text-emerald-600 mt-1">{confirmed}</p></div>
          <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm"><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Avg Risk</p><p className="text-3xl font-black text-slate-900 mt-1">{avgScore}<span className="text-sm font-bold text-slate-400">/10</span></p></div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          <div className="flex bg-slate-200/50 p-1 rounded-2xl w-fit border border-slate-200">
            {(['pending', 'confirmed', 'cancelled', 'all'] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`px-5 py-2 rounded-xl text-sm font-bold capitalize transition-all ${activeTab === tab ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>{tab}</button>
            ))}
          </div>
