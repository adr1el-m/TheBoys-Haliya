...
export default function FacilityDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "pending" | "confirmed" | "cancelled" | "all"
  >("pending");
  ...
  const updateStatus = async (id: string, status: string) => {
    try {
      const res = await fetch(
        `${API_URL}/appointments/${id}/status?status=${status}`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${user?.token}` },
        },
  ...
  const avgScore =
    appointments.length > 0
      ? (
          appointments.reduce(
            (sum: number, a: any) => sum + (a.triage_score || 0),
            0,
          ) / appointments.length
        ).toFixed(1)
      : "—";
      ...
      <aside className="w-72 bg-white border-r border-slate-100 p-8 hidden lg:flex flex-col gap-8">
        ...
        <div className="bg-slate-50 rounded-2xl p-5 space-y-3 border border-slate-100">
          ...
          <div className="grid grid-cols-3 gap-1 pt-2 border-t border-slate-200">
            ...
            <div className="text-center">
              <p className="text-lg font-black text-emerald-500">{confirmed}</p>
              <p className="text-[9px] font-bold text-slate-400 uppercase">
                Done
              </p>
            </div>
        ...
        <nav className="space-y-2">
          ...
          <Link
            href="/public-health?from=facility"
      ...
      <div className="flex-1 p-6 md:p-12 overflow-y-auto">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
          <div>
            ...
            <p className="text-slate-500 font-medium">
              Healthcare Facility Dashboard • Central Management
            </p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={fetchData}
              className="p-4 bg-white text-slate-400 rounded-2xl hover:text-blue-600 transition-all border border-slate-100 shadow-sm"
            >
              <RefreshCw size={18} />
            </button>
            <div className="bg-blue-600 px-5 py-3 rounded-2xl flex items-center gap-2 text-white font-bold shadow-xl shadow-blue-100 text-sm">
              <Activity size={16} />
              Live Queue
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          <div className="flex bg-slate-200/50 p-1 rounded-2xl w-fit border border-slate-200">
            {(["pending", "confirmed", "cancelled", "all"] as const).map(
              (tab) => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`px-5 py-2 rounded-xl text-sm font-bold capitalize transition-all ${activeTab === tab ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                >
                  {tab}
                </button>
              ),
          ...
          <div className="relative w-full sm:w-auto">
            ...
            <input
              ...
              className="w-full sm:w-72 bg-white border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-700 focus:ring-2 focus:ring-blue-500 outline-none transition-all shadow-sm"
            />
          </div>
        </div>
                  ...
                  filtered.map((appt) => (
                    ...
                    >
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm ${getUrgencyBg(appt.triage_score)} ${getUrgencyText(appt.triage_score)}`}
                          >
                            {appt.patient_name?.[0] || "?"}
                          </div>
                          <div>
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-2">
                          <div
                            className={`w-2.5 h-2.5 rounded-full ${getUrgencyDot(appt.triage_score)}`}
                          />
                          <span
                            className={`text-sm font-black ${getUrgencyText(appt.triage_score)}`}
                          >
                      ...
                      <td className="px-6 py-5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            className="p-2 bg-slate-50 text-slate-500 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition-all"
                            title="View"
                          >
                            <Eye size={16} />
                          </button>
                          ...
                          {appt.status !== "pending" && (
                            <span
                              className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full ${getStatusStyle(appt.status)}`}
                            >
                              {appt.status}
                            </span>
                          )}
        ...
        {viewAppt && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-[2rem] p-8 w-full max-w-lg shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto"
            >
              ...
              <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl">
                <div
                  className={`w-14 h-14 rounded-xl flex items-center justify-center font-black text-xl ${getUrgencyBg(viewAppt.triage_score)} ${getUrgencyText(viewAppt.triage_score)}`}
                >
                  {viewAppt.patient_name?.[0]}
                </div>
                <div>
                  <p className="font-bold text-slate-800 text-lg">
                    {viewAppt.patient_name}
                  </p>
                ...
                <div className="p-4 bg-slate-50 rounded-2xl">
                  ...
                  <p className="text-sm font-bold text-slate-800 mt-1">
                    {viewAppt.appointment_date
                      ? new Date(viewAppt.appointment_date).toLocaleDateString(
                          "en-PH",
              ...
              {viewAppt.triage_score && (
                <div className="p-4 bg-slate-50 rounded-2xl">
                  ...
                  <p className="text-[11px] font-bold uppercase text-slate-400 mt-1">
                    {getUrgencyLabel(viewAppt.triage_score)}
                  </p>
                </div>
              )}
              ...
              {viewAppt.status === "pending" && (
                <div className="flex gap-3 pt-2">
                  <button
                    onClick={() => {
                      updateStatus(viewAppt.id, "confirmed");
                      setViewAppt(null);
                    }}
                  ...
                  <button
                    onClick={() => {
                      updateStatus(viewAppt.id, "cancelled");
                      setViewAppt(null);
                    }}
